...
def train(epoch, net, net2, optimizer, labeled_trainloader, unlabeled_trainloader,  op, savelog=False):
    ...
    while (cont_iters < max_iters):  # longmix
        for batch_idx, (inputs_x, inputs_x2, inputs_x3, inputs_x4, labels_x, w_x, indices_x) in enumerate(
            ...
            inputs_x, inputs_x2, inputs_x3, inputs_x4, labels_x, w_x = inputs_x.cuda(), inputs_x2.cuda(), \
                inputs_x3.cuda(), inputs_x4.cuda(), labels_x.cuda(), w_x.cuda()

            inputs_u, inputs_u2, inputs_u3, inputs_u4 = inputs_u.cuda(), inputs_u2.cuda(), inputs_u3.cuda(), \
                inputs_u4.cuda()
...
if __name__ == '__main__':
    name_exp = 'PLR_single_penalty%d_cn%d' % (args.yespenalty, args.num_clean)
    ...
    scheduler1 = optim.lr_scheduler.CosineAnnealingLR(optimizer1, 380, 2e-4)
    scheduler2 = optim.lr_scheduler.CosineAnnealingLR(optimizer2, 380, 2e-4)
    ...
    for epoch in range(resume_epoch, args.num_epochs + 1):
        ...
        if epoch < warm_up:
            ...
            # 计算每个类别准确率
            inds_clean11 = np.asarray([ind for ind in (pred1).nonzero()[0] if noisy_labels[ind] == clean_labels[ind]])
            inds_clean22 = np.asarray([ind for ind in (pred2).nonzero()[0] if noisy_labels[ind] == clean_labels[ind]])
            sel_idx1 = []
            sel_idx2 = []
            pred_len = [[], []]
            for i in range(args.num_class):
                temp_len = len(np.where(noisy_labels[pred1.nonzero()[0]] == i)[0])
                pred_len[0].append(temp_len)
                sel_idx1.append(len(np.where(noisy_labels[inds_clean11] == i)[0]) / temp_len)
                temp_len = len(np.where(noisy_labels[pred2.nonzero()[0]] == i)[0])
                pred_len[1].append(temp_len)
                sel_idx2.append(len(np.where(noisy_labels[inds_clean22] == i)[0]) / temp_len)
            print(f'\nBefore uniform, total clean num={len(inds_clean11)}')
            print(
                f'\nBefore uniform, total clean num={len(inds_clean22)}')
            print(
                f'\nBefore uniform, each class clean num = {sel_idx1},\t {sel_idx2}')
            print(f'Before uniform, each class num={pred_len[0]}\t {pred_len[1]}')
            ...
            for i in range(args.num_class):
                ...
                pred_len[1].append(temp_len)
                sel_idx2.append(len(np.where(noisy_labels[inds_clean22] == i)[0]) / temp_len)
            print(f'\nafter uniform, total clean num={len(inds_clean11)}')
            print(
                f'\nafter uniform, total clean num={len(inds_clean22)}')
            print(
                f'\nafter uniform, each class clean num = {sel_idx1},\t {sel_idx2}')
            print(f'after uniform, each class num={pred_len[0]}\t {pred_len[1]}')


            print('Train Net1')
            labeled_trainloader, unlabeled_trainloader, _ = loader.run('train', pred2, prob2,
                                                                       second_ind=second_ind)  # co-divide
            train(epoch, net1, net2, optimizer1, labeled_trainloader, unlabeled_trainloader, op2['op'], savelog=True)
            # 这里还需要噪声修正
            # device = torch.device("cuda")
            ...
            labeled_trainloader, unlabeled_trainloader, u_map_trainloader = loader.run('train', pred1, prob1,
                                                                                       second_ind=second_ind)  # co-divide
            train(epoch, net2, net1, optimizer2, labeled_trainloader, unlabeled_trainloader, op1['op'], savelog=False)  # train net2
            # 这里还需要噪声修正
            # device = torch.device("cuda")
